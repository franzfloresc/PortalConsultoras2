  ...
  SELECT pwd.CampaniaID,
      ...
      CASE
		...
		(
			...
			SELECT TOP 1 '[ ' + TE.DescripcionEstrategia + ' ]'
			...
			WHERE
			(E.CampaniaID = pwd.CampaniaID OR pwd.CampaniaID BETWEEN E.CampaniaID AND CASE WHEN E.CampaniaIDFin = 0 THEN E.CampaniaID ELSE E.CampaniaIDFin END)
			AND TE.FlagActivo = 1
			AND E.CUV2 = pwd.CUV
		)
...
BEGIN
	...
	SELECT 
		...
		TE.DescripcionEstrategia as DescripcionEstrategia, --R2469
		CASE 
			WHEN TE.FlagNueva = 1 THEN
				CASE WHEN @ProgramaNuevoActivado > 0 THEN 1 END  
			ELSE 0 
		END  AS TipoEstrategiaID, -- R2621 -- R2621
		PC.IndicadorOferta AS IndicadorOfertaCUV,  /*R20150701LR*/
		CASE WHEN ISNULL(scd.PedidoWebDetalleID,0) <> 0 THEN 1 ELSE 0 END FlagConsultoraOnline,
		PW.DescuentoProl,
		PW.MontoEscala,
