...
-- login con nro de documento
IF (ISNULL(@TipoUsuario,0) = 0)
BEGIN
	DECLARE @CodigoUsuario_2 VARCHAR(30)
	SELECT 
		@CodigoConsultora = ISNULL(CodigoConsultora,''),
		@CodigoUsuario_2 = CodigoUsuario,
		@PaisID = ISNULL(PaisID,0),
		@TipoUsuario = ISNULL(TipoUsuario,0)
	FROM usuario WITH(NOLOCK)
	WHERE DocumentoIdentidad LIKE @CodigoUsuario
	AND ClaveSecreta = @Contrasenia

	SET @CodigoUsuario = @CodigoUsuario_2
END
