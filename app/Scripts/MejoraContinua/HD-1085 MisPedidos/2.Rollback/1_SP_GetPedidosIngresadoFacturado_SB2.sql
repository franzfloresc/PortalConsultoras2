...
BEGIN
	...
	WHERE 
		...
		and	(select count(*) from PedidoWebDetalle(NOLOCK) where CampaniaID = PW.CampaniaID and ConsultoraID = PW.ConsultoraID
	) <> 0
	...
	WHERE co.ConsultoraID=@ConsultoraID	and	P.CampaniaID <> @CampaniaID
	GROUP BY P.PedidoID,CA.Codigo,P.MontoFacturado,P.Origen,P.Flete,p.FechaFacturado
