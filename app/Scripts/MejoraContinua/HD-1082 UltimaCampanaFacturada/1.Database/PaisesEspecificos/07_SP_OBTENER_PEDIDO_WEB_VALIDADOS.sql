USE BelcorpBolivia
GO
ALTER PROCEDURE dbo.OBTENER_PEDIDO_WEB_VALIDADOS 
...
BEGIN
	...
	WHERE
		...
		AND pdw.EstadoPedido = 202 AND pdw.ValidacionAbierta = 0 AND pdw.ModificaPedidoReservadoMovil = 0
...
BEGIN
	...
	FROM PedidoWeb pdw
		...
		LEFT JOIN ods.SOA_INFOR_PEDID sip WITH(NOLOCK) ON 
			c.ConsultoraID = sip.ConsultoraID
			AND sip.COD_PERI = @CodigoCampaniaActual
			AND sip.FEC_ULTI_ACTU=(select max(FEC_ULTI_ACTU) FROM ods.SOA_INFOR_PEDID where COD_CLIE=sip.COD_CLIE AND COD_PERI=sip.COD_PERI)
		LEFT JOIN ods.Territorio terr WITH(NOLOCK) ON 
			c.TerritorioID = terr.TerritorioID 
			AND c.SeccionID = terr.SeccionID
		LEFT JOIN ods.Telefono tf WITH(NOLOCK) ON 
