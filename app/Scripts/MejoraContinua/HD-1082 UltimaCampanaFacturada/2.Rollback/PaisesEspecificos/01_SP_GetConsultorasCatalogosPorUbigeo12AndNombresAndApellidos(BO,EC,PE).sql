...
BEGIN
	...
	AND CONCAT(PrimerNombre,' ',SegundoNombre) LIKE CONCAT('%',@Nombres,'%')
	AND CONCAT(PrimerApellido,' ',SegundoApellido) LIKE CONCAT('%',@Apellidos,'%')
	AND (@MarcaId <> 3 OR year(C.FechaNacimiento) > @AnioLimite) 
END
GO