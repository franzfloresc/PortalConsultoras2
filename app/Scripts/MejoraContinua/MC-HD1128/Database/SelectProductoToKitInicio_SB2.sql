...
IF EXISTS(
	...
	WHERE SPECIFIC_NAME = 'SelectProductoToKitInicio_SB2' AND SPECIFIC_SCHEMA = 'dbo' AND Routine_Type = 'PROCEDURE'
)
BEGIN
...
BEGIN
	...
	LEFT JOIN Estrategia EST ON EST.CampaniaID = p.AnoCampania 
							 AND (EST.CUV2 = p.CUV OR EST.CUV2 = (SELECT CUVPadre FROM TallaColorCUV TCC WHERE TCC.CUV = p.CUV)) 
							 AND EST.Activo = 1 
	LEFT JOIN dbo.TallaColorCUV tcc ON  tcc.CUV = p.CUV 
