...
BEGIN TRAN
	...
	FROM(
		SELECT TOP (@CANTIDAD_TOP)
			...
			SUBSTRING(I.NUMERO, len(I.NUMERO)-3,4) AS CLAVEGENERADA,
		...
		LEFT JOIN ODS.IDENTIFICACION I ON C.CONSULTORAID = I.CONSULTORAID AND I.DocumentoPrincipal = '1' AND I.TipoDocumento = 'CCI'
		LEFT JOIN USUARIO U ON C.CODIGO = U.CODIGOCONSULTORA
		LEFT JOIN ODS.ZONA Z ON C.ZONAID = Z.ZONAID
		WHERE U.CODIGOCONSULTORA IS NULL AND LEN(I.NUMERO) > 4
		AND ISNULL(I.NUMERO,'')<>''
	) T
