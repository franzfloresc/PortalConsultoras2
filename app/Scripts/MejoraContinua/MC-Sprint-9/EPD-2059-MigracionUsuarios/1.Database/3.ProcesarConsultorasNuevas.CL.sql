...
CREATE PROCEDURE PROCESARCONSULTORASNUEVAS --@CANTIDAD_TOP INT,@CODIGOISO VARCHAR(3),@CONSULTORAONLINE INT 
	@CANTIDAD_TOP INT,
	@CODIGOISO VARCHAR(3) ,
...
BEGIN TRAN
	...
	SELECT CONSULTORAID,CODIGOUSUARIO,CODIGOCONSULTORA,@PAISID PAISID,	
		   ...
		   SOBRENOMBRE,COMPARTIRDATOS,ACTIVO,TIPOUSUARIO,CAMBIOCLAVE,GZEMAIL,GERENTEZONA,CODIGOZONA,ZONAID,FECHAINGRESO	
	FROM(
		SELECT TOP (@CANTIDAD_TOP)
			C.CONSULTORAID,C.CODIGO CODIGOUSUARIO,C.CODIGO CODIGOCONSULTORA,
			...
			UPPER(SUBSTRING(master.dbo.fn_varbintohexstr(HashBytes('SHA1', UPPER(CONVERT(NVARCHAR, I.NUMERO)))),3,40)) CLAVE,
			I.NUMERO CLAVEGENERADA, 
		...
		LEFT JOIN ODS.IDENTIFICACION I ON C.CONSULTORAID = I.CONSULTORAID  AND I.DOCUMENTOPRINCIPAL = '1' AND I.TipoDocumento = 'RUT'
	...
	IF @CANTIDAD_REGISTROS>0
			BEGIN 
				...
				FROM @TB_INFORMACION T WHERE NOT EXISTS(SELECT 1 FROM DBO.USUARIO WHERE codigousuario = T.NUMERO_IDENTIFICACION)
				...
				SELECT CODIGOUSUARIO,1,1 FROM @TB_INFORMACION T WHERE NOT EXISTS(SELECT 1 FROM DBO.USUARIO WHERE codigousuario = T.NUMERO_IDENTIFICACION)
				...
				UPDATE SolicitudPostulante
				SET CodigoConsultora = T.CODIGOCONSULTORA,
				FechaIngreso = T.FECHA_INGRESO,
				EstadoPostulante = 5
				FROM @TB_INFORMACION T INNER JOIN SolicitudPostulante S ON T.NUMERO_IDENTIFICACION = S.NUMERODOCUMENTO
									
				UPDATE DBO.USUARIO set CODIGOCONSULTORA = T.CODIGOCONSULTORA
				FROM @TB_INFORMACION T INNER JOIN DBO.USUARIO S ON T.CODIGOCONSULTORA=S.DOCUMENTOIDENTIDAD AND IDESTADOACTIVIDAD NOT IN (5,7)
