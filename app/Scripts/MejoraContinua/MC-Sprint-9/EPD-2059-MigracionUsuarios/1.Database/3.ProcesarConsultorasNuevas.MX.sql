...
BEGIN TRAN
	...
	FROM(
		SELECT TOP (@CANTIDAD_TOP)
			...
			UPPER(SUBSTRING(master.dbo.fn_varbintohexstr(HashBytes('SHA1', UPPER(CONVERT(NVARCHAR, SUBSTRING(C.CODIGO, LEN(C.CODIGO)-3,4))))),3,40)) CLAVE,
			SUBSTRING(C.CODIGO, len(C.CODIGO)-3,4) AS CLAVEGENERADA,
		...
		LEFT JOIN ODS.IDENTIFICACION I ON C.CONSULTORAID = I.CONSULTORAID AND RTRIM(LTRIM(I.DocumentoPrincipal)) = '1' AND I.TipoDocumento = 'RFC B'
		LEFT JOIN USUARIO U ON C.CODIGO = U.CODIGOCONSULTORA
		LEFT JOIN ODS.ZONA Z ON C.ZONAID = Z.ZONAID
		WHERE U.CODIGOCONSULTORA IS NULL AND LEN(I.NUMERO) > 6
		AND ISNULL(I.NUMERO,'')<>''
	) T
