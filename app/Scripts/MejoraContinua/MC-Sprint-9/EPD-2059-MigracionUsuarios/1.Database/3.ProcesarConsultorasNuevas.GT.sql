...
BEGIN TRAN
	...
	FROM(
		SELECT TOP (@CANTIDAD_TOP)
			...
			UPPER(SUBSTRING(master.dbo.fn_varbintohexstr(HashBytes('SHA1', UPPER(CONVERT(NVARCHAR, substring(I.NUMERO, len(I.NUMERO)-3,4))))),3,40)) CLAVE,
			substring(I.NUMERO, len(I.NUMERO)-3,4) CLAVEGENERADA,
		...
		WHERE U.CODIGOCONSULTORA IS NULL AND LEN(I.NUMERO) > 3 
		AND ISNULL(I.NUMERO,'')<>''
	) T
