...
inner join ServicioCampania SC with(nolock) ON S.ServicioId = SC.ServicioId  
...
AND  SC.PaisId = @PaisId AND
SCC.SegmentoInternoID IS NOT NULL
...
SELECT S.ServicioId, S.Descripcion, S.Url, ISNULL(SCC.Segmento,-1) as Segmento, ISNULL(SCC.ConfiguracionZona,'') as ConfiguracionZona  
 FROM Servicio S with(nolock)  
 inner join ServicioCampania SC with(nolock) ON S.ServicioId = SC.ServicioId  
 left join ServicioCampaniaSegmentoZona SCC with(nolock) ON SC.ServicioId = SCC.ServicioId AND  
                SC.CampaniaId = SCC.CampaniaId AND  
                SC.PaisId = SCC.PaisId  
 WHERE  
  SC.CampaniaId = @CampaniaId and  
  SC.PaisId = @PaisId  
AND SCC.SegmentoInternoID IS null 
END 
