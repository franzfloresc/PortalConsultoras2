...
insert into @tempFRS
select max(dtrs.SugeridoID) as SugeridoID
...
where dtrs.tipo = 'FRS'
group by dtrs.CampaniaID, dtrs.ConsultoraID, dtrs.CUVOriginal
...
UNION ALL
select dtrs.campaniaid AS campaniaid, @FechaFact_ as FechaRegistro,pr.codigoproducto,pr.codigotipooferta,
dtrs.cantidad*pr.FactorRepeticion as cantidad, dtrs.cantidad * dtrs.PrecioUnidad as PrecioUnidad
from [dbo].[DemandaTotalReemplazoSugerido] dtrs with(nolock)
...
inner join ods.productocomercial pr with(nolock) on ca.campaniaid = pr.campaniaid and acc.cuv = pr.cuv
UNION ALL
select dtrs.campaniaid AS campaniaid, @FechaFact as FechaRegistro,pr.codigoproducto,pr.codigotipooferta,
dtrs.cantidad*pr.FactorRepeticion as cantidad, dtrs.cantidad * dtrs.PrecioUnidad as PrecioUnidad
from [dbo].[DemandaTotalReemplazoSugerido] dtrs with(nolock)
inner hash join @tempFRS tr on dtrs.SugeridoID = tr.DemandaTotalReemplazoSugeridoId
inner join ods.campania ca with(nolock) on dtrs.campaniaid = ca.codigo
