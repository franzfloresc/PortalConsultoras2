...
AS
/*
exec dbo.GetProductoComercialByDescripcionByFilter 201818,5,'kalos',2701,2161,'50','5052'
...
BEGIN
 ...
 SELECT DISTINCT TOP (@_RowCount)
  ...
  ,op.TipoOfertaSisID
  ,te.flagNueva
  ,EST.EstrategiaID as EstrategiaID,
  EST.codigoEstrategia as CodigoEstrategia
  ,te.TipoEstrategiaID
  ,P.IndicadorOferta
 ...
 LEFT JOIN Estrategia EST WITH (NOLOCK)
  ...
  AND (EST.CUV2 = p.CUV)
  AND EST.Activo = 1
  AND (EST.NumeroPedido = 0 or EST.NumeroPedido = @NumeroPedido)
 LEFT JOIN TipoEstrategia TE WITH (NOLOCK)
  ON TE.TipoEstrategiaID = EST.TipoEstrategiaID
  AND (TE.CodigoPrograma = '' or TE.CodigoPrograma = @CodigoPrograma)
