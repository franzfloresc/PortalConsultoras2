...
/****** Object:  StoredProcedure [dbo].[ListarOfertasMasVendidos]    Script Date: 20/11/2018 15:38:55 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		WHERE
			...
			AND c.ConsultoraID IN (@ConsultoraID,@codConsultoraForzadas)
	...
	BEGIN
		...
		, TE.DescripcionEstrategia AS DescripcionEstrategia
		, ISNULL(TE.FlagMostrarImg,0) AS FlagMostrarImg
		,CASE WHEN TF.CUV IS NULL THEN 0 ELSE 1 END
		FROM Estrategia E
		...
		WHERE
			E.Activo = 1
			AND ca.Codigo = @CampaniaID
			AND op.CodConsultora IN (@ConsultoraGenerica,@codConsultoraForzadas)
			AND E.Zona LIKE '%' + @ZonaID + '%'
			AND TE.FlagActivo = 1
			AND TE.flagRecoPerfil = 1
			--AND E.CUV2 not in ( SELECT CUV FROM @tablaCuvFaltante )
	...
	LEFT JOIN ProductoComentario PC on T.CodigoProducto = PC.CodigoSap AND PC.Estado = 1) T
	ORDER BY
