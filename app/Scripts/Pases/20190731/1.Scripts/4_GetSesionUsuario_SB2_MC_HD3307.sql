...
-- exec GetSesionUsuario_SB2 '007528493'
...
BEGIN
	...
	select @ZonaID = IsNull(ZonaID,0),
		...
		@IndicadorConsultoraDigital = ISNULL(IndicadorConsultoraDigital,0)
	...
	IF @UsuarioPrueba = 0
	BEGIN
		SET @PasePedidoWeb = 0 --(SELECT dbo.GetPasaPedidoWeb(@CampaniaID,@ConsultoraID))
		SET @TipoOferta2 = 0 --(SELECT dbo.GetComproOfertaWeb(@CampaniaID,@ConsultoraID))
		SET @CompraOfertaEspecial = 0 --(SELECT dbo.GetComproOfertaEspecial(@CampaniaID,@ConsultoraID))
		SET @IndicadorMeta = 0 --(SELECT dbo.GetIndicadorMeta(@ConsultoraID))
		...
		SELECT
			...
			,ISNULL(c.PagoContado, 0) AS PagoContado /*HD-4513*/
	...
	BEGIN
		SELECT
			...
			,ISNULL(scl.DescripcionSubClasificacion, '') AS DescripcionSubClasificacion /*HD-3777*/
			,ISNULL(c.AutorizaPedido, 0) AS AutorizaPedido /*HD-3693*/
			,ISNULL(cons.PagoContado, 0) AS PagoContado /*HD-4513*/
