...
BEGIN
	...
	WHERE TE.Codigo = '022'
	AND EST.Activo = 1
	AND @CodigoCampania BETWEEN CampaniaID AND CampaniaIDFin
	...
	(
		SELECT 
			...
			EST.ImagenURL AS ImagenPremio,
			ROW_NUMBER()OVER(ORDER BY NIV.CodigoConcurso, NIV.CodigoNivel, PRE.COD_PREM, EST.Orden) AS ID
		...
		INNER JOIN @TblConcurso CON
		ON CON.CodigoConcurso = NIV.CodigoConcurso
		INNER JOIN @TblEstrategia EST
		ON EST.CUV2 = PRE.COD_PREM
		...
		SELECT 
			...
			LTRIM(RTRIM(dbo.InitCap(PRE.DES_PROD))) AS DescripcionPremio,
			PRE.NUM_PREM AS NumeroPremio,
			NULL AS ImagenPremio,
			ROW_NUMBER()OVER(ORDER BY NIV.CodigoConcurso, NIV.CodigoNivel, PRE.COD_PREM) AS ID
		FROM ods.IncentivosPremiosNivel PRE (NOLOCK)
		INNER JOIN @TblNiveles NIV
		ON PRE.COD_CONC = NIV.CodigoConcurso
		AND PRE.NUM_NIVE = NIV.CodigoNivel
		WHERE NOT EXISTS(SELECT 1 FROM @TblEstrategia EST WHERE EST.CUV2 = PRE.COD_PREM)
	)
	SELECT 
		CodigoConcurso,
		CodigoNivel,
