...
SELECT bp.BannerID, bp.PaisID, ISNULL(sgf.SegmentoInterno,-1) as Segmento, ISNULL(ConfiguracionZona,'') as ConfiguracionZona  
from dbo.BannerPais bp with(nolock)  
left join dbo.BannerPaisSegmentoZona bpsz with(nolock) on bp.CampaniaId = bpsz.CampaniaId and bp.BannerId = bpsz.BannerId and 
bp.PaisId = bpsz.PaisId  
left join   @segmentofinal sgf on sgf.BannerID = bp.BannerID  and sgf.paisID= bp.PaisID /*Cambio C2544 - Agregar paisID*/
where bp.CampaniaID = @CAMPANIAID   
and  bpsz.SegmentoInterno IS NOT NULL  
...
bp.PaisId = bpsz.PaisId    
where bp.CampaniaID = @CAMPANIAID
AND bpsz.SegmentoInterno IS null   
order by bp.BannerID, bp.PaisID  /*Cambio 2544 - Correccion de Banner Inferiores*/
END
