...
IF EXISTS(SELECT 1 FROM sys.procedures 
          WHERE object_id = OBJECT_ID(N'dbo.GetNotificacionesConsultora'))
BEGIN
...
BEGIN
	...
	FROM
		...
		(SELECT 	
			...
			when 'MONTO MÍNIMO'  then 'Pedido rechazado por monto mínimo'
			when 'MONTO MAXIMO'  then 'Pedido rechazado por monto máximo'
			when 'MONTO MINIMO STOCK'  then 'Pedido rechazado por monto mínimo'
			when 'MONTO MAXIMO- DEUDA'  then 'Pedido rechazado por monto máximo y deuda'
			when 'MONTO MINIMO STOCK- DEUDA'  then 'Pedido rechazado por monto mínimo y deuda'
			when 'DEUDA- MONTO MÍNIMO'  then 'Pedido rechazado por monto mínimo y deuda'
			else ''
			END)
			AS Asunto,
