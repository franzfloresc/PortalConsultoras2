...
BEGIN  
 ...
 BEGIN  
  ...
  FROM ( SELECT CampaniaId, CodigoOferta, NumeroGrupo, PrecioUnitario=MIN(PrecioUnitario), IndicadorPreUni=MAX(IndicadorPreUni),  FactorRepeticion, CodigoFactorCuadre  
    FROM #ProductoComercial  
    WHERE EstrategiaIdSicc = 2003  
    GROUP BY CampaniaId, CodigoOferta, NumeroGrupo,  FactorRepeticion, CodigoFactorCuadre) PC  
  GROUP BY PC.CampaniaId, PC.CodigoOferta  
  ORDER BY PC.CampaniaId, PC.CodigoOferta  
