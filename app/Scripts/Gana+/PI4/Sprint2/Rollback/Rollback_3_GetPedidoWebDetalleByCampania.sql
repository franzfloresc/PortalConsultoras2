...
BEGIN
	...
	INNER JOIN ods.Consultora co ON co.Codigo = se.CodigoCliente
		AND co.esconsultoralider = 1
	...
	BEGIN
		...
		FROM (
			...
			LEFT JOIN @tblSuscripcionSE SSE ON PWD.CampaniaID = SSE.CampaniaID
				AND PWD.CUV = SSE.CUV --/* HD-4200 */
			...
			LEFT JOIN dbo.Cliente C ON PWS.ClienteID = C.ClienteID
				AND PWS.ConsultoraID = C.ConsultoraID
