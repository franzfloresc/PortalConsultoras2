...
BEGIN
	...
	BEGIN
		...
		left join ods.Identificacion i with(nolock) on i.ConsultoraID = c.ConsultoraID and i.DocumentoPrincipal = 1
		WHERE ro.Sistema = 1 and u.CodigoUsuario = @CodigoConsultora;
	END
	ELSE
	BEGIN
		SELECT
		...
		left join ods.Identificacion i with(nolock) on i.ConsultoraID = cons.ConsultoraID and i.DocumentoPrincipal = 1
		WHERE ro.Sistema = 1 and u.CodigoUsuario = @CodigoConsultora;
	END
END
GO