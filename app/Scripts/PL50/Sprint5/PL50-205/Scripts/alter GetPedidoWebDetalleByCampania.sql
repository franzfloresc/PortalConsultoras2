...
begin
				...
				INNER JOIN PedidoWeb PW ON   PW.CampaniaId = PWS.Campania and PWS.PedidoID = PW.PedidoID --PWS.PedidoID = PW.PedidoID
				inner join PedidoWebSetDetalle PWSD on PWSD.SetID=PWS.SetID
				INNER JOIN PedidoWebDetalle PWD ON  PW.CampaniaId = PWD.CampaniaId and PWD.PedidoID = PW.PedidoID and PWSD.CuvProducto =PWD.CUV --PWD.PedidoID = PW.PedidoID and PWSD.CuvProducto =PWD.CUV
