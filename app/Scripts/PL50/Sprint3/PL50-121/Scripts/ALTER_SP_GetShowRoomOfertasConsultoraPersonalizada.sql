USE BelcorpBolivia
GO
  IF OBJECT_ID('ShowRoom.GetShowRoomOfertasConsultoraPersonalizada', 'P') IS NOT NULL
	DROP PROC ShowRoom.GetShowRoomOfertasConsultoraPersonalizada
GO
 --[ShowRoom].[GetShowRoomOfertasConsultoraPersonalizada] 201804, '000001740'  
CREATE PROCEDURE ShowRoom.GetShowRoomOfertasConsultoraPersonalizada 
  @CampaniaID INT  
...
BEGIN  
 ...
 WHERE c.Codigo = @CampaniaID  
  ...
  /*consultora generica*/
  IF NOT EXISTS (SELECT 1 FROM  ods.ofertaspersonalizadas op WHERE op.CodConsultora= @CodigoConsultora AND op.AnioCampanaVenta= @CampaniaID 
  AND op.TipoPersonalizacion= 'SR')
  BEGIN
    SELECT @CodigoConsultora = Codigo FROM TablaLogicaDatos WHERE TablaLogicaDatosID = 10001
  END
  /*end generica*/
