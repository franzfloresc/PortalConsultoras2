...
'2000-01-01' as FechaFacturacion, 'Validación de Stock' as Asunto , 0 as FacturaHoy, 0 as Visualizado, 0 as EsMontoMinino,0 as 'FlagProcedencia', 1
...
BEGIN
	...
	FROM
		(SELECT 	
			LGPRV.LogGPRValidacionId  AS ProcesoId, --ProcesoValidacionPedidoRechazadoId 
			ISNULL(LGPRV.Campania, '201601')   AS CampaniaId,'' AS EstadoPROL,
			LGPRV.FechaFinValidacion AS FechaHoraValidación,
			CAST(0 AS BIT) as EnvioCorreo,
			'PEDREC' AS Proceso,
			1 AS Estado,
			...
			when ISNULL(LGPRV.DescripcionRechazo, '') = 'OCC-51,OCC-19' OR ISNULL(LGPRV.DescripcionRechazo, '') = 'OCC-19,OCC-51' then 'Pedido rechazado por monto mínimo y deuda' when ISNULL(LGPRV.DescripcionRechazo, '') = 'OCC-16,OCC-19' OR ISNULL(LGPRV.DescripcionRechazo, '') = 'OCC-19,OCC-16' then 'Pedido rechazado por monto mínimo y deuda'
			else ''
			END)
			AS Asunto,
			CAST(0 AS BIT) AS FacturaHoy,
			...
			1 AS FlagProcedencia,
			row_number() over (partition by LGPRV.ProcesoValidacionPedidoRechazadoId order by LGPRV.ProcesoValidacionPedidoRechazadoId) as row_number
		FROM GPR.LogGPRValidacion LGPRV WHERE LGPRV.Rechazado = 1 AND LGPRV.ConsultoraId = @ConsultoraId)
	as rows 
	where row_number = 1
