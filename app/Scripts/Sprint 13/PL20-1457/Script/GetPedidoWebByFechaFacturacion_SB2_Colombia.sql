--exec dbo.GetPedidoWebByFechaFacturacion_SB2 @FechaFacturacion='2017-05-24 00:00:00',@TipoCronograma=1,@NroLote=1883
use BelcorpColombia
go
...
--Cupon
	...
	select 
		...
		p.IPUsuario,
		TipoCupon = Isnull((select top 1 TipoCupon From #pedido_cupon pedido_cupon Where pedido_cupon.CampaniaID = p.CampaniaID And pedido_cupon.PedidoID = p.PedidoID ),'00'),
		ValorCupon = Isnull((select top 1 ValorCupon From #pedido_cupon pedido_cupon Where pedido_cupon.CampaniaID = p.CampaniaID And pedido_cupon.PedidoID = p.PedidoID ),'000000000000')
	from dbo.PedidoWeb p with(nolock)
