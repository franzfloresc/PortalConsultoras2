...
BEGIN
	...
	TipoCupon = '00', ValorCupon = '000000000000' --valores default
	...
	Begin 
		...
		From @tabla_pedido pedido
			Inner join Cupon with (nolock) On pedido.CampaniaId = Cupon.CampaniaId
			Inner join CuponConsultora with (nolock) On 
				...
				pedido.CodigoConsultora = CuponConsultora.CodigoConsultora
			Inner Join Usuario with (nolock) on 
				Usuario.CodigoConsultora = pedido.CodigoConsultora
			Inner Join @tabla_pedido_detalle pedido_detalle on 
			...
			Inner Join ods.ProductoComercial producto with (nolock) on 
				pedido_detalle.CUV = producto.CUV And
				pedido_detalle.CampaniaId = producto.AnoCampania										
		Where 
			Cupon.Estado = 1 And 
			CuponConsultora.EstadoCupon = 2 And 			
	...
	select 
		...
		p.IPUsuario,
		TipoCupon ,ValorCupon --columnas nuevas
	from @tabla_pedido p
