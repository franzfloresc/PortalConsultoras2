...
if exists (select 1 from sysobjects where id = object_id('dbo.Cupon') and type = 'U')
   drop table dbo.Cupon
go

if exists (select 1 from sysobjects where id = object_id('dbo.CuponConsultora') and type = 'U')
...
if (select COUNT(*) from dbo.sysobjects inner join dbo.syscolumns on SYSOBJECTS.ID = SYSCOLUMNS.ID 
	where sysobjects.id = object_id('dbo.Pais') and SYSCOLUMNS.NAME = N'TieneCupon') = 1
	ALTER TABLE dbo.Pais drop column TieneCupon
go

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetCuponConsultoraByCodigoConsultoraCampaniaId]') AND type in (N'P', N'PC')) 
	DROP PROCEDURE [dbo].GetCuponConsultoraByCodigoConsultoraCampaniaId
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[UpdateCuponConsultoraEstadoCupon]') AND type in (N'P', N'PC')) 
	DROP PROCEDURE [dbo].UpdateCuponConsultoraEstadoCupon
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[UpdateCuponConsultoraEnvioCorreo]') AND type in (N'P', N'PC')) 
	DROP PROCEDURE [dbo].UpdateCuponConsultoraEnvioCorreo
GO
