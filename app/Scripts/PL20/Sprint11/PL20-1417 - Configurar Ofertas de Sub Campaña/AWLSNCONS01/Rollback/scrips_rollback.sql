...
if (select COUNT(*) from dbo.sysobjects inner join dbo.syscolumns on SYSOBJECTS.ID = SYSCOLUMNS.ID 
	where sysobjects.id = object_id('ShowRoom.Evento') and SYSCOLUMNS.NAME = N'TieneSubCampania') = 1
	ALTER TABLE ShowRoom.Evento DROP COLUMN TieneSubCampania
go

if (select COUNT(*) from dbo.sysobjects inner join dbo.syscolumns on SYSOBJECTS.ID = SYSCOLUMNS.ID 
	where sysobjects.id = object_id('ShowRoom.OfertaShowRoom') and SYSCOLUMNS.NAME = N'EsSubCampania') = 1
	ALTER TABLE ShowRoom.OfertaShowRoom DROP COLUMN EsSubCampania
go
