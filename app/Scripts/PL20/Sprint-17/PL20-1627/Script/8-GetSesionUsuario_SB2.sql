...
BEGIN
	/******************PL20-1627****************/	
	declare @OfertaFinal int /*********Existe Oferta final por Pais******/
	set @OfertaFinal = 0;

	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('','','','',@CodigoConsultora) where Codigo in ('OFT','OFC') and Estado = 1)
	set @OfertaFinal = 1;
	
	declare @OFGanaMas tinyint /*******Existe Oferta Final Gana Mas*****/
	set @OFGanaMas = 0
	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('','','','',@CodigoConsultora) where Codigo in ('OFTGM') and Estado = 1)
	set @OFGanaMas = 1
	/******************PL20-1627****************/
	...
	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('',@CodigoRegion,@CodigoZona,'',@CodigoConsultora) where Codigo in ('OFT','OFC') and Estado = 1)
	...
	BEGIN
		...
		SELECT
			...
			@OfertaFinal as OfertaFinal,--1627
			...
			@OFGanaMas AS OfertaFinalGanaMas,
...
BEGIN
	...
	BEGIN
		...
		SELECT
			...
			u.EMailActivo, --2532
			si.SegmentoInternoId,
			isnull(@OfertaFinal,0) as OfertaFinal,
			isnull(@EsOfertaFinalZonaValida,0) as EsOfertaFinalZonaValida,
			@FechaLimitePago as FechaLimitePago,
			ISNULL(p.CatalogoPersonalizado,0) as CatalogoPersonalizado,
			ISNULL(u.VioVideo, 0) as VioVideo,
			...
			ISNULL(p.TieneHana,0) as TieneHana,
			z.GerenteZona,
			ISNULL(@OFGanaMas,0) AS OfertaFinalGanaMas,
			ISNULL(@EsOFGanaMasZonaValida,0) AS EsOFGanaMasZonaValida,
			isnull(c.IndicadorBloqueoCDR,0) as IndicadorBloqueoCDR,
			isnull(@EsCDRWebZonaValida,0) as EsCDRWebZonaValida,
			isnull(p.TieneCDR,0) as TieneCDR,
...
BEGIN
	...
	declare @OFGanaMas tinyint /*******Existe Oferta Final Gana Mas*****/
	set @OFGanaMas = 0
	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('','','','',@CodigoConsultora) where Codigo in ('OFCGM') and Estado = 1)
	set @OFGanaMas = 1
	/******************PL20-1627****************/
	DECLARE @PasePedidoWeb int
	DECLARE @TipoOferta2 int
	...
	declare @CodigoZona varchar(4) = ''
	declare @CodigoRegion varchar(2) = ''
	declare @CodigoSeccion varchar(8) = ''

	select
		@CodigoZona = IsNull(z.Codigo,''),
		@CodigoRegion = IsNull(r.Codigo,''),
		@CodigoSeccion = IsNull(s.Codigo,'')
	from ods.consultora c with(nolock)
	inner join ods.Region r on
		c.RegionID = r.RegionID
	inner join ods.Zona z on
	...
	/******************PL20-1627****************/	
	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('',@CodigoRegion,@CodigoZona,@CodigoSeccion,@CodigoConsultora) where Codigo in ('OFC') and Estado = 1)
	set @OfertaFinal = 1;
	/******************PL20-1627****************/

	if exists(select 1 from dbo.fnConfiguracionPais_GetAll('',@CodigoRegion,@CodigoZona,'',@CodigoConsultora) where Codigo in ('OFC') and Estado = 1)
		set @EsOfertaFinalZonaValida = 1
