...
begin
	...
	where @Codigo <> '' and Codigo = @Codigo
	...
	and 
	(
		(@confiPaisId > 0 AND ConfiguracionPaisID = @confiPaisId) OR
		(
	...
	WHERE (@confiPaisId > 0 AND DD.ConfiguracionPaisID = @confiPaisId) OR
		(
		 DD.CodigoRegion = @CodigoRegion
