...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 FROM Estrategia where CUV2 = @CUV2 AND CampaniaID = @CampaniaID) SET @flag = 99
	...
	ELSE IF @flag = 4 OR @flag = 9 OR @flag = 10 OR @flag = 11 OR @flag = 12 OR @flag = 13
	BEGIN
		-- VALIDAR QUE EL CUV EXISTA PARA LA CAMPAÑA ACTUAL
		...
		IF (@flag = 13) AND NOT EXISTS (SELECT 1 
			FROM ods.ProductoComercial PC
			INNER JOIN ods.Campania C ON PC.CampaniaID = C.CampaniaID
			WHERE C.codigo = @CampaniaID AND CUV = @CUV2 AND CodigoTipoOferta = '126' AND FactorRepeticion = 1)
		BEGIN 
			RAISERROR('El CUV2 ingresado no tiene el código de herramienta de venta.', 16, 1)
		END 

		SELECT
			COALESCE(MC.DescripcionOriginal, PD.Descripcion, PC.Descripcion) AS DescripcionCUV2,
			PC.PrecioUnitario,
			mc.IdMatrizComercial,
		...
		FROM ODS.PRODUCTOCOMERCIAL PC
			INNER JOIN ODS.CAMPANIA C ON PC.CAMPANIAID = C.CAMPANIAID
			LEFT JOIN ProductoDescripcion PD on PC.AnoCampania = PD.CampaniaID and PC.CUV = PD.CUV
			LEFT JOIN MATRIZCOMERCIAL MC ON PC.CODIGOPRODUCTO = MC.CODIGOSAP
		WHERE
