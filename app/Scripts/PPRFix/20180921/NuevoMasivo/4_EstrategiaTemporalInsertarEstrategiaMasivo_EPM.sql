...
ALTER PROCEDURE EstrategiaTemporalInsertarEstrategiaMasivo
(
	@NroLote int = 0,
...
BEGIN
	...
	FROM Estrategia E
		INNER JOIN EstrategiaProducto EP
			...
			AND E.CUV2 = EP.CUV2
	WHERE EXISTS(
				SELECT CampaniaId FROM EstrategiaTemporal ET 
				WHERE ET.CampaniaId = E.CampaniaId 
				AND ET.NumeroLote = @NroLote
				and dbo.fnGetTipoEstrategiaId(ET.CodigoTipoEstrategia) = E.TipoEstrategiaID 
		)
