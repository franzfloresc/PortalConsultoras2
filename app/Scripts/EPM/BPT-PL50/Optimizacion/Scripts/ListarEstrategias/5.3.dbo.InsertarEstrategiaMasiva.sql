...
(CUV2, DescripcionCUV2, Precio, Precio2, LimiteVenta, TextoLibre, EsSubCampania, UsuarioCreacion, FechaCreacion, 
TipoEstrategiaID, CampaniaID, EtiquetaID,EtiquetaID2, Activo, CampaniaIDFin, NumeroPedido, FlagDescripcion, CUV, FlagCEP, 
FlagCEP2, Cantidad, FlagCantidad, ColorFondo, Ganancia)    
SELECT DISTINCT  
      tabla.variable.value('CUV2[1]', 'VARCHAR(20)') AS 'CUV2'    
    , tabla.variable.value('DescripcionCUV2[1]', 'VARCHAR(800)') AS 'DescripcionCUV2'    
    , tabla.variable.value('Precio[1]', 'NUMERIC(12,2)') AS 'Precio'    
    , tabla.variable.value('Precio2[1]', 'NUMERIC(12,2)') AS 'Precio2'    
    , tabla.variable.value('LimiteVenta[1]', 'INT') AS 'LimiteVenta'    
 ...
 FROM @EstrategiaXML.nodes('/strategy/row') tabla(variable) INNER JOIN ods.OfertasPersonalizadasCUV op 
    ON op.CUV = tabla.variable.value('CUV2[1]', 'VARCHAR(20)')    
