...
BEGIN
	...
	BEGIN
		...
		FROM @tablaCuvsOPT t 
		INNER JOIN Estrategia e ON e.CampaniaID = t.CampaniaID AND e.CUV2 = t.CUV
		WHERE e.CampaniaID = @CampaniaID AND e.TipoEstrategiaID = @TipoEstrategiaId
		GROUP BY t.CUV) AS A)
	END
	ELSE
	BEGIN
		...
		FROM @tablaCuvsOPT t 
		LEFT JOIN Estrategia e on t.CampaniaID = e.CampaniaID AND t.CUV = e.CUV2 AND e.TipoEstrategiaID = @TipoEstrategiaId
