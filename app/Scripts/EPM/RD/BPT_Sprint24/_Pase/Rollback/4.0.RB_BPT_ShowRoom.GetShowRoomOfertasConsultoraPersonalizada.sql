...
/*Validacion FaltanteAnunciado y ProductoFaltante*/
	...
	FROM Estrategia e
		...
		INNER JOIN ods.OfertasPersonalizadas op
			ON c.codigo = op.anioCampanaVenta
			AND e.CUV2 = op.CUV
			AND op.TipoPersonalizacion = 'SR'
		INNER JOIN vwEstrategiaShowRoomEquivalencia ves
			ON e.TipoEstrategiaId = ves.TipoEstrategiaID
	WHERE c.Codigo = @CampaniaID
		AND op.CodConsultora = @CodigoConsultora
		AND e.Activo = 1
		AND e.CUV2 NOT IN (SELECT CUV FROM @tablaFaltante)
	ORDER BY op.Orden
