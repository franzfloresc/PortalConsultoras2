...
BEGIN
	...
	BEGIN TRY
		...
		IF @CampaniaIDFin > 0
		BEGIN
			IF EXISTS(SELECT 1 FROM dbo.Estrategia (NOLOCK) WHERE CUV2 = @CUV2 AND ((@CampaniaID BETWEEN CampaniaID AND CampaniaIDFin) OR (@CampaniaIDFin BETWEEN CampaniaID AND CampaniaIDFin)) AND TipoEstrategiaID = @TipoEstrategiaID AND EstrategiaID <> @Estrategi
aID  AND NumeroPedido = @NumeroPedido)
			BEGIN
		...
		IF NOT EXISTS(SELECT 1 FROM dbo.Estrategia (NOLOCK) WHERE EstrategiaID = @EstrategiaID)
		BEGIN	
