...
BEGIN  
	...
	BEGIN  
		...
		SET @CodUltimaCampFact = (select campaniaid from ods.campania where codigo = @UltimaCampanaFacturada )		
			...
			DECLARE @CampaniaSiguiente VARCHAR(6)
			DECLARE @CampaniaSiguienteID INT
			SELECT @CampaniaSiguiente= dbo.fnGetCampaniaSiguiente (@UltimaCampanaFacturada)
			SET	@CampaniaSiguienteID = (select campaniaid from ods.campania where codigo = @CampaniaSiguiente) 
