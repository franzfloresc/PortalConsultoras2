	...
	SET NOCOUNT ON
		BEGIN TRY
			...
			BEGIN
				...
				IF NOT EXISTS(SELECT 1 FROM ods.ProductoComercial PC INNER JOIN ods.Campania C ON PC.CampaniaID = C.CampaniaID 
				...
				IF NOT EXISTS(SELECT 1 FROM ODS.PRODUCTOCOMERCIAL 
				WHERE 
						CUV = @CUV2
					AND AnoCampania = @CampaniaID
					AND CODIGOTIPOOFERTA IN (SELECT CodigoOferta FROM TipoEstrategiaOferta TEO INNER JOIN Oferta O ON O.OfertaID = TEO.OfertaID
				...
				BEGIN
					...
					IF NOT EXISTS(SELECT 1 FROM ods.ProductoComercial PC 
				...
				SELECT @FlagNueva = FlagNueva, @FlagRecoProduc = FlagRecoProduc, @FlagRecoPerfil = FlagRecoPerfil FROM TipoEstrategia WHERE TipoEstrategiaID = @TipoEstrategiaID
				...
				WHERE 
					CODIGOTIPOOFERTA IN (SELECT CODIGOOFERTA 
										FROM 
											TIPOESTRATEGIAOFERTA TE 
											INNER JOIN OFERTA O ON O.OFERTAID = TE.OFERTAID 
										WHERE 
											TIPOESTRATEGIAID = @TipoEstrategiaID OR 0 = @FlagOferta)
			...
			BEGIN
				...
				DECLARE @codigoSAP VARCHAR(20)
				SET @codigoSAP = (SELECT CodigoProducto 
									FROM ods.ProductoComercial pc 
									INNER JOIN ods.Campania c ON PC.CAMPANIAID = C.CAMPANIAID 
									WHERE C.Codigo = @CampaniaID AND CUV = @CUV2)
				...
				SELECT PC.CUV,PC.Descripcion DescripcionCUV2, PC.PrecioUnitario, '' FotoProducto01, '' FotoProducto02, '' FotoProducto03
				FROM ODS.PRODUCTOCOMERCIAL PC 
					INNER JOIN ODS.CAMPANIA C ON PC.CAMPANIAID = C.CAMPANIAID
				WHERE 
					C.CODIGO = @CampaniaID
			...
			BEGIN
				...
				DECLARE @numeroTallas INT
				SET @numeroTallas = (SELECT COUNT(1) FROM TallaColorCUV WHERE CampaniaID = @CampaniaID AND CUVPadre = @CUV2)
				...
				IF @numeroTallas > 1 
				BEGIN
					SELECT '' CUV, '' DescripcionCUV2, COUNT(1) PrecioUnitario, '' FotoProducto01, '' FotoProducto02, '' FotoProducto03  
					FROM TallaColorCUV 
					WHERE 
				...
				ELSE
				BEGIN
					SELECT '' CUV, '' DescripcionCUV2, -1 PrecioUnitario, '' FotoProducto01, '' FotoProducto02, '' FotoProducto03
				END
			END
		END TRY
		BEGIN CATCH
