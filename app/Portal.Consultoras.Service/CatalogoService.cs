    ...
    public class CatalogoService : ICatalogoService
    {
        ...
        public List<BEConsultoraCatalogo> GetConsultorasPorCodigoTerritorioGeo(string codigoPais, string codigoTerritorioGeo)
        {
            try
            {
                ...
                if (idPais == 0) throw new ClientInformationException("El código de Pais recibido no es válido.");
                if (codigoTerritorioGeo.Length != 13) throw new ClientInformationException("El codigo de TerritorioGeo recibido no es válido.");
            ...
            catch (Exception ex)
            {
                ...
                throw new ClientInformationException("Ocurrió un error al intentar obtener las consultoras.");
            }
        }
        ...
        private List<BEConsultoraCatalogo> GetConsultorasCatalogos(string codigoPais, string codigoUbigeo, int idPais, int marcaId, string nombres, string apellidos, int tipoFiltroUbigeo)
        {
                ...
                || codigoPais == Constantes.CodigosISOPais.Bolivia)
            {
                ...
                else throw new ClientInformationException("La longitud del parámetro CodigoUbigeo debe ser 12 o 18");
            }
            else
            {
                ValidarCodigoUbigeo(idPais, codigoUbigeo);
