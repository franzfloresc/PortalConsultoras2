...
function accionesCampoBusquedaAlDigitar() {
    $('body').on('keyup', '#CampoBuscadorProductos', function () {
        ...
        if (cantidadCaracteresParaMostrarSugerenciasBusqueda > 0) {
            $('.enlace_busqueda_filtros').fadeOut(150);
            $('.opcion_limpiar_campo_busqueda_productos').delay(50);
            $('.opcion_limpiar_campo_busqueda_productos').fadeIn(150);
        } else {
            $('.opcion_limpiar_campo_busqueda_productos').fadeOut(150);
            $('.enlace_busqueda_filtros').delay(50);
            $('.enlace_busqueda_filtros').fadeIn(150);
        }

        if (cantidadCaracteresParaMostrarSugerenciasBusqueda >= CaracteresBuscador) {
            console.log($('#CampoBuscadorProductos').val());
        }
    });
