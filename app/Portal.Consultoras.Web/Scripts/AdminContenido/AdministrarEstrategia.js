var AdministrarEstrategia = (function (config) {
        ...
        return function (data, textStatus, jqXHR) {
           ...
           if (data.success == false) {
            ...
            if (aux1 == "4" || aux2 == "005" || aux2 == "007" || aux2 == "008" || aux2 == "030") {
                ...
                $("#txt1-estrella").html("Oferta de òltimo Minuto");
                $("#txt2-estrella").hide();
            } else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.listActualizado == 0 && data.listInsertado == 0) {
                    ...
                    mensaje = "El procedimiento culmino con Žxito, <br /> - Se actualizaron " + data.listActualizado + " Set(s) <br /> - Se insertaron " + data.listInsertado + " Set(s)";
                }
                closeWaitingDialog();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.listActualizado == 0) {
                    ...
                    mensaje = "El procedimiento culmino con Žxito, se actualizaron " + data.listActualizado + " producto(s)";
                }
                closeWaitingDialog();
                $("#estadoCargaMasiva").css('color', 'black');
                $('#estadoCargaMasiva').html(mensaje);
            ...
            buttons:
            {
                "Guardar": function () {
                    ...
                    if (jQuery.trim($("#txtEventoDiasDespues").val()) == "")
                        vMessage += "- Debe ingresar la cantidad de dias despuŽs de la Facturaci—n.\n";
                    if (parseInt(jQuery.trim($("#txtEventoDiasDespues").val())) <= 0)
                        vMessage += "- La cantidad de dias despuŽs de la Facturaci—n debe ser mayor a cero.\n";
    ...
    var _eventos = {
        ...
        clickActivarDesactivar: function () {
            var proceder = confirm("À Desea habilitar/deshabiltar las estrategias?");
            if (!proceder)
                return false;
    ...
    function Deshabilitar(id, event) {
        ...
        var elimina = confirm("ÀEst‡ seguro que desea deshabiltar la estrategia seleccionada?");
        if (!elimina)
            return false;
    ...
    function Remover(id, event) {
        event.preventDefault();
        event.stopPropagation();
        var elimina = confirm("ÀEst‡ seguro que desea eliminar el set seleccionado?");
        if (!elimina) {
            return false;
        }
        if (id) {
    ...
    function DeshabilitarEvento(ID, CampaniaID) {
        var deshabilitar = confirm("À Est‡ seguro que desea deshabilitar el evento?");
        if (!deshabilitar)
            return false;
    ...
    function EliminarEvento(ID, CampaniaID) {
        var eliminar = confirm("À Est‡ seguro que desea eliminar el evento?");
        if (!eliminar)
            return false;
    ...
    function EliminarProducto(EstrategiaID, event) {
        ...
        var eliminar = confirm("À Est‡ seguro que desea deshabilitar todos los productos del set ?");
        if (!eliminar)
            return false;
    ...
    function EliminarProductoDetalle(ID, EstrategiaID, CUV) {
        var eliminar = confirm("À Est‡ seguro que desea deshabilitar el producto ?");
        if (!eliminar)
            return false;
