...
namespace Portal.Consultoras.Web.Controllers
{
    public class CatalogoController : BaseController
    {
                    ...
                    foreach (var item in ListaCatalogosCliente)
                    {
                        ...
                        if (item.Esika == "1")
                        {
                            ...
                            else
                            {
                                ...
                                mailBody += "<td width=\"98\" style=\"text-align:center;\"><a href=\"" + "http://www.esika.biz" + "\" style=\"color:#333;\">Esika</a></td>";
                        }
                        if (item.Cyzone == "1")
                        {
                            if (catalogoCyZone != null && !string.IsNullOrEmpty(catalogoCyZone.DocumentID))
                                ...
                                mailBody += "<td width=\"98\" style=\"text-align:center;\"><a href=\"" + "http://www.cyzone.com" + "\" style=\"color:#333;\">Cyzone</a></td>";
        ...
        [HttpPost]
        public JsonResult GetCatalogosPublicadosJson(string campaniaId)
        {
            List<CatalogoIssuModel> listCatalogoIssuModel = new List<CatalogoIssuModel>();
            List<Catalogo> listCatalogo = this.GetCatalogosPublicados(UserData().CodigoISO, campaniaId);
            foreach (Catalogo catalogo in listCatalogo)
            {
                listCatalogoIssuModel.Add(new CatalogoIssuModel{
                    IdMarcaCatalogo = (int)catalogo.IdMarcaCatalogo,
                    DocumentID = catalogo.DocumentID,
                    SkinURL = catalogo.SkinURL
                });
            }
            ...
            return Json(new {
                Success = true,
                Message = "",
                Catalogos = listCatalogoIssuModel
            });
        }
        ...
        private List<Catalogo> GetCatalogosPublicados(string paisISO, string campaniaId)
        {
            List<Catalogo> catalogos = new List<Catalogo>();
