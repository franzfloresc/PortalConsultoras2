        ...
        public List<BEFichaProducto> GetFichaProducto(BEFichaProducto entidad)
        {
            ...
            if (esFacturacion)
            {
                ...
                fichasProductos.ForEach(fichaProducto =>
                {
                    ...
                    {
                        var add = true;
                        if (fichaProducto.TipoImagenMostrar ==
                            Constantes.TipoEstrategia.OfertaParaTi)
                        {
                        add = listaTieneStock.Any(p => p.Codsap.ToString() == fichaProducto.CodigoProducto && p.estado == 1);
                        }
                        if (!add) return;

                        if (fichaProducto.Precio >= fichaProducto.Precio2)
                            fichaProducto.Precio = Convert.ToDecimal(0.0);
