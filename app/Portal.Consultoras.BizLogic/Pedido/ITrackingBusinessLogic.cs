...
namespace Portal.Consultoras.BizLogic
{
    public interface ITrackingBusinessLogic
    {
        ...
        List<BETracking> GetPedidosByConsultora(int paisID, string codigoConsultora, int top, int tipoDoc = 0);
        List<BEPostVenta> GetSeguimientoPostVenta(int paisID, string numeroRecojo, int estadoRecojoID);
        List<BETracking> GetTrackingByPedido(int paisID, string codigo, string campana, string nropedido, int tipoDoc = 0);
